...
`download-siteone.sh` downloads only that versioned HTTPS release asset, validates its upstream-published SHA-256 before extraction or execution, confirms the verified archive contains the expected executable, and installs it into the ignored local tools directory. The immutable release asset path plus verified digest establishes the exact artifact identity. Pull-request validation stops at artifact verification; the crawler is first executed by an authorized scan job.
